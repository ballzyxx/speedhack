...
### Changed
- Module identity is `speedhack`. Chat command is `/8 spd` (alias `speedhack`). GUI hotkey is Ctrl+Shift+S.
- Ground loc test is type **2** (fall) only. At 3.0x, types **0, 1, 3, 4, 5, 6, 7, 8** all kick. Fly modes still reset on login.
- Run/walk stay type **2**. Jump is separate: **hide** (floor as type 2), real type 5, or its own type (`/8 spd jumptype`).
- Real mount-fly keeps type 2 on the ground. `C_PLAYER_FLYING_LOCATION` is rewritten to flying type **7** (Mode 4 air rule) without turning Modes 1–4 on.

### Added
- **Ground loc types** — rewrite `C_PLAYER_LOCATION.type` and keep the packet as a ground loc. Fly modes 1–4 were flying packets; this path was never tried.
- GUI dropdown and `/8 spd loctype <0..8|off>`. Off = real type. Exclusive with fly modes. Run/walk/jump (0 / 1 / 5 / 8) are rewritten; stop stays real.
- GUI jump dropdown and `/8 spd jumptype <0..8|off|hide>`. Hide flattens the jump arc onto the floor as type 2.

---
## 2.62 (2026-09-12)

### Changed
- Ground mount steps are capped to the last on-foot cruise speed (same idea as air boost). Mode 4 at 3.0x on a mount no longer trips the 10s average.

---
## 2.61 (2026-09-12)

### Added
- **Mode 4** — foot/ground type **7**, mount and fly type **7**. 3.0x on foot stayed connected; type 8 at 3.0x kicked.

---
## 2.60 (2026-09-12)

### Added
- Exclusive **Mode 1 / 2 / 3** ticks (one at a time, off by default).
- Mode 1 foot type **1**, Mode 2 foot type **8**, Mode 3 foot type **3**. Mount and real fly stay type **7** on all three.

### Changed
- Air boost no longer sends the real boost step. It stays type 7 and is capped to the last cruise-fly speed so the 10s average does not kick.

---
## 2.59 (2026-09-12)

### Added
- **Flying-loc mode** — opt-in GUI tick / `/8 spd flytest`. Off = normal ground packets.
- Foot run uses fly type **1** (needs more testing). Ground mount and real fly use type **7**.
- Stop / skill sends a real ground plant so you stay on the floor.
- `/8 spd rl` reloads the module in-game so type/code edits apply without leaving the world. `/8 spd reloadcfg` only rereads settings.

---
## 2.58 (2026-09-08)

### Fixed
- Live HP tracking uses the same bigint/number-safe id compare as movement, so a toggle no longer stamps a stale bar.
- Enable then disable before the first STAT packet now applies off, not the queued on.
- `S_EXIT` kick reports are no longer overwritten by a later `leave_game` logout.
- `spd reload` re-arms the AHK hotkey after rereading `config.json`.

---
## 2.57 (2026-09-08)

### Fixed
- Blank hotkey stays off. It is no longer forced back to `-` on load.
- `spd reset` re-arms the default `-` hotkey instead of leaving AHK stopped.
- Combat auto-disable hides the buff icon while speed is suspended, and shows it again when combat ends.
- AHK restart no longer busy-waits the proxy for 80ms (uses a timer instead).
- Leftover `gatherSpeed` config key is stripped. Gathering is still not multiplied.
- If `S_USER_MOVETYPE` is missing or arrives before gameId, locomotion is seeded from the live STAT block so on/off still refreshes walk/run.
